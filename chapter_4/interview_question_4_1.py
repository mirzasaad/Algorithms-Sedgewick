...
import random
from module_4_1 import Graph, BreadthFirstPaths, ConnectedComponent
...
class Eulerian(object):
    ...
    def isEuler(self):
        ...
        return EulerianType.EULER_CIRCUIT if number_of_odd_vertex_degree == 0 else EulerianType.EULER_PATH

    def odd_vertices(self):
        number_of_odd__vertex_degree = 0

        for vertex in self._graph.vertices():
            if len(self._graph.degree(vertex)) & 1:
                number_of_odd__vertex_degree += 1

        return number_of_odd__vertex_degree

    def get_random_vertex_with_odd_degree(self):
        return random.sample([v for v in self._graph.vertices() if self._graph.degree(v) & 1], 1)

    def get_random_vertex(self):
        return random.sample(self._graph.vertices(), 1)
    ...
    def trail(self):
        ...
        ) if isEuler == EulerianType.CIRCUIT else self.get_random_vertex_with_odd_degree()
