...
from numpy import Infinity, empty, number
...
class ShortestPath(metaclass=ABCMeta):
    def relax_edge(self, graph: EdgeWeightedDigraph, edge: DirectedEdge):
    ...
    def relax_vertex(self, graph: EdgeWeightedDigraph, vertex: DirectedEdge):
...
use toppological sort if no directed cycles
use DijkstraSP if no negative weights
use BellmanFord if no negative cycle
...
#NOTE DijkstraSP works if no nnegative weight
...
class DijkstraAllPairsSP(object):
    ...
    def __init__(self, graph: EdgeWeightedDigraph):
        ...
        for vertex in graph.vertices():
            self._all[vertex] = DijkstraSP(graph, vertex)
...
#NOTE Topological sort work with negativge weights, but no directed cycle
...
class ParalledJobsScheduling(object):
    """
    ...
    def __init__(self, schedules, offet = 0) -> None:
        durations, successors = schedules['durations'], schedules['successors']

        self._vertices = set()
        self._graph = EdgeWeightedDigraph()
        self._offset = offet
        ...
        for vertex, duration in durations.items():
            self._vertices.add(vertex)

            self._graph.add_edge(DirectedEdge(vertex, vertex + N, duration))
            self._graph.add_edge(DirectedEdge(self._start, vertex, 0.0))
            self._graph.add_edge(DirectedEdge(vertex + N, self._end, 0.0))

        for vertex in successors.keys():
            for successor in successors[vertex]:
                self._graph.add_edge(DirectedEdge(vertex + N, successor, 0.0))

        self._alp = AcyclicLP(self._graph, self._start)

    def job_start_times(self):
        for vertex in self._vertices:
            yield (vertex, self._offset + self._alp.dist_to(vertex))

    def job_finish_time(self):
        return self._offset + self._alp.dist_to(self._end)
...
class BellmanFordSPNonQueueBased(ShortestPath):
    """
    loop 0 to Number of vertices
        for every vertex relax its adjacent edges
    """
    def __init__(self, graph: EdgeWeightedDigraph, source):
        self._dist_to = dict((v, INFINITE_POSITIVE_NUMBER) for v in graph.vertices())
        self._dist_to[source] = 0
