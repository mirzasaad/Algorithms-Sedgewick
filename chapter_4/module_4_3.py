...
"""
minimum spanning tree is graph with positive weighted edges and is connnected and is asylic

a cut in a graph is parition of its vertices into non empty sets
a crosssing edge connects 2 different sets or 2 vertices from two parition
and that crossing edge has to be of minimum weight
"""
...
class WeightedGraphMatrix(object):
    ...
    >>> wgm
    8 vertices, 16 edges.
    4: 4-5 0.35, 4-7 0.37, 4-0 0.38, 4-6 0.93
    5: 5-4 0.35, 5-7 0.28, 5-1 0.32
    7: 7-4 0.37, 7-5 0.28, 7-0 0.16, 7-1 0.19, 7-2 0.34
    0: 0-7 0.16, 0-4 0.38, 0-2 0.26, 0-6 0.58
    1: 1-5 0.32, 1-7 0.19, 1-2 0.36, 1-3 0.29
    2: 2-3 0.17, 2-0 0.26, 2-1 0.36, 2-7 0.34, 2-6 0.4
    3: 3-2 0.17, 3-1 0.29, 3-6 0.52
    6: 6-2 0.4, 6-3 0.52, 6-0 0.58, 6-4 0.93
    <BLANKLINE>
    """
...
class PrimMST(object):
    ...
    [6-2 0.4, 0-2 0.26, 4-5 0.35, 0-7 0.16, 1-7 0.19, 5-7 0.28, 2-3 0.17]
    """

    def __init__(self, graph, start_vertex):
        self._edge_to = {}
        self._dist_to = dict((vertex, 999999999)
    ...
    def visit(self, graph: EdgeWeightedGraph, vertex):
        self._marked[vertex] = True
        ...
        for edge in graph.adjacent_edges(vertex):
            ...
            if edge.weight < self._dist_to[other_vertex]:
                ...
                """
                we put all all the adjacent vertices on pq with edge weight as a value
                
                we chooose a adjacent edge and check if the other vertex on edge is visited before if it was visited before
                then it means the that the edge that connects that vertex is in pq and and is no longer use full as the min dis edge is already
                picked so we replace the the vertex value with new edge

                if the other vertex has not been visited put in pq
                """

                if not self._pq.contains(other_vertex):
                    self._pq.insert(other_vertex, self._dist_to[other_vertex])
                else:
                    self._pq.change_key(
                        other_vertex, self._dist_to[other_vertex])
...
class EdgeConnectedComponent(object):
    ...
    >>> ecc.count()
    3
    >>> sorted([sorted(comp) for comp in ecc.get_components()])
    ...
    def dfs(self, graph: EdgeWeightedGraph, vertex):
        self._marked[vertex] = True
...
class EdgeSetMST(object):
    ...
    """
    # >>> sorted([edge for edge in mst.edges()])
    # [0-7 0.16, 1-7 0.19, 5-7 0.28, 6-2 0.4, 3-6 0.52, 6-0 0.58, 6-4 0.93]
